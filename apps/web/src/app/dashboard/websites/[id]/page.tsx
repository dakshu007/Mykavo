...
export default async function WebsiteDetailPage({
  ...
  return (
    <div className="space-y-6">
            ...
            {activeScan ? (
              <>
                ...
                <Link
                  ...
                  className="inline-flex h-11 items-center gap-2 rounded-full bg-primary-soft px-6 text-sm font-medium text-accent"
                >
                  <span
                    aria-hidden
            ...
            {website.monitoredPages.map((page) => {
              ...
              return (
                <li key={page.id}>
                  ...
                  >
                    <span className="min-w-0 flex-1 truncate font-mono text-[13px] text-ink group-hover:text-accent">
                      {path === "/" ? "/ (homepage)" : path}
                    </span>
                    {baselineVersion ? (
      ...
      <Card>
        <CardHeader icon={Activity} title="Health" />
        {latestHealth === null ? (
      ...
      {/* Configuration, folded away.
          Eight settings cards previously sat in the same scroll, at the same
          visual weight, as live findings - so half this page was things you
          touch once. Collapsed by default: <details> keeps it keyboard
          accessible and linkable without needing a route. */}
      <details className="group rounded-card bg-card shadow-card">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-6 [&::-webkit-details-marker]:hidden">
          ...
          <span className="flex items-center gap-2 text-[13px] text-ink-secondary">
            Tags, alerts, comparison, status page, reports
            <ChevronDown
              aria-hidden
        ...
        <div className="space-y-6 border-t border-line p-6 pt-6">
      <Card>
