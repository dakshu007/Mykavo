...
import { followRedirectChain } from "@/lib/tools/redirect-chain";
...
export async function POST(request: Request) {
  const limit = rateLimit(`redirect-chain:${clientKey(request)}`, {
    limit: 10,
    windowMs: 60_000,
  });
  if (!limit.allowed) {
  ...
  try {
    const chain = await followRedirectChain(parsed.href);
    return NextResponse.json({ chain });
  } catch (err) {
    if (err instanceof UnsafeUrlError) {
      return NextResponse.json({ error: SAFE_FETCH_USER_MESSAGES[err.code] }, { status: 400 });
    }
    console.error("[redirect-chain] unexpected failure", err);
    return NextResponse.json(
      { error: "Something went wrong checking that URL. Please try again." },
      { status: 500 },
    );
  }
}
